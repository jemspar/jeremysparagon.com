...
/*     var animation_pos_1a = 300; //where you want the animation to begin
    ...
    var colors = [darkGreen, maroon, darkBlue, pink, black, darkGreen, deepBlue, darkGray, lightBlue, darkTeal];

    var lineColors = [maroon, darkBlue, pink, new $.Color('rgb(181,201,212)'), white, darkGreen, grayBlue, new $.Color('rgb(0,82,0)'), new $.Color('rgb(76,77,63)'), new $.Color('rgb(64,104,166)'), new $.Color('rgb(253,190,187)'), new $.Color('rgb(90,0,0)'), new $.Color('rgb(223,242,247)'), new $.Color('rgb(89,61,163)'), new $.Color('rgb(157,183,112)'), new $.Color('rgb(0,73,93)')];
    ...
    for (let index = 0; index < lineColors.length; index++) {
        ...
        .css({ "animation-delay" : index/6 + "s",
                "transform" : "translateX(50px), rotate(23deg), translateY(" + index*20 + "px)",
    ...
    function scrollLinkedEffects() {
        ...
        var ch2_pos = $('.white_sands_text').position().top + $('body').height()/3;
        var ch2_h = $('.white_sands_text').height()*2 + $('.white_sands_bg').height();
        var ch4_pos = $('.ch4_flex').position().top;
        var ch4_h = $('.ch4_flex').height();
        var ch3_middle = $('.ch3_flex').position().top + ($('.ch3_flex').height()*3/8);
        ...
        var ch7_end = ch7_start + $('.ch7_container').height() - $('body').height()*1.5;
        ...
        var pos = [
            ...
            ch1_pos+$('.ch1_img.moved').position().top+$('.ch1_img.moved').height(),
            kira_pos,
            ch1_pos+ch1_h,
            ch2_pos,
            ch4_pos-ch4_h,
        ...
        if (leftbound_pos_index % 2 == 0) {
            $('body, .white_sands_text.first, .kira_img').animate({ backgroundColor: colors[leftbound_pos_index/2] }, 0);
            // $('.ch1_img.kira').animate({backgroundColor: colors[leftbound_pos_index/2]}, 0);
        }
        else {
            var percentScrolled =  (0 - pos[leftbound_pos_index])  / 
            ...
            var newRed = colors[startColor].red() + ( ( colors[endColor].red() - colors[startColor].red() ) * percentScrolled );
            var newGreen = colors[startColor].green() + ( ( colors[endColor].green() - colors[startColor].green() ) * percentScrolled );
            var newBlue = colors[startColor].blue() + ( ( colors[endColor].blue() - colors[startColor].blue() ) * percentScrolled );
            var newColor = new $.Color( newRed, newGreen, newBlue );
            //console.log( newColor.red(), newColor.green(), newColor.blue() );
            $('body, .white_sands_text.first, .kira_img').animate({ backgroundColor: newColor }, 0);
        ...
        if ((ch7_start + $('.ch7_container').height()/4 < 0 && ch7_end > 0) && ch7_scroll == 0) {
            ch7_scroll = 1;
        ...
        } else if ((ch7_start + $('.ch7_container').height()/4 >= 0 || ch7_end < 0 ) && ch7_scroll == 1) {
